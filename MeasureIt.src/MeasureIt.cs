...
public static class MeasureIt
{
    ...
    static public int Main(string[] args)
    {
        ...
        for (int i = 0; i < args.Length; i++)
        {
            ...
            else if (string.Compare(args[i], "/Edit", StringComparison.OrdinalIgnoreCase) == 0)
            {
                ...
                string vs2017 = Path.Combine(programFiles, "Microsoft Visual Studio", "2017");
            ...
            else if (string.Compare(args[i], "/?", StringComparison.OrdinalIgnoreCase) == 0)
            {
                ...
                Console.WriteLine("    /usersGuide          Display the users guide.  ");
    ...
    private static Stats RunTests(List<MethodInfo> areas)
    {
        ...
        timer100000 = new MultiSampleCodeTimer(10, 100000);
        timer100000.OnMeasure += logger.AddWithCount;
    ...
    static public void MeasureSaulehRegExComp()
    {
        int idx = 0;
        timer1000.Measure("RegexMatchConstructor", delegate
        {
            idx += new Regex("a.*b").Match("aaacccb").Length;
        });

        idx = 0;
        Regex r = new Regex("a.*b");
        timer1000.Measure("RegexMatch_WithoutConstructor", delegate
        {
            idx += r.Match("aaacccb").Length;
        });

        idx = 0;
        r = new Regex("a.*b", RegexOptions.Compiled);
        timer1000.Measure("RegexMatch_WithoutConstructor_Compiled", delegate
        {
            idx += r.Match("aaacccb").Length;
        });
    ...
    private static MultiSampleCodeTimer timer100000;
